## Movie database
#### Create and delete a movie database
``` cypher
// Clear graph
MATCH (n)
DETACH DELETE n;
...
RETURN count (*)
```
**Delete a database. The delete is permanent so use with caution:**
``` cypher
MATCH (n:Person|Movie)
DETACH DELETE n
```
#### Adding constraints for nodes
...
- **Search-performance indexes:** for speeding up data retrieval based on _exact_ matches. This category includes range, text, point, and token lookup indexes.
- **Semantic indexes:** for _approximate_ matches and to compute similarity scores between a query string and the matching data. This category includes full-text and vector indexes.
``` cypher
...
```
#### Find nodes
**Find a person Tom Hanks as a node in the graph:**
``` cypher
MATCH (p:Person {name: "Tom Hanks"})
RETURN p
```
**Find a movie Cloud Atlas as a node in the graph:**
``` cypher
MATCH (m:Movie {title:"Cloud Atlas"})
RETURN m
```
**Find 10 persons as nodes in the graph:**
``` cypher
MATCH (p:Person)
RETURN p LIMIT 10
```
**Find 10 persons as nodes in the graph and display their names as a table:**
``` cypher
MATCH (p:Person)
RETURN p.name LIMIT 10
```
**Find movie nodes with specific filter on years of production:**
``` cypher
...
```
**Return all nodes connected to Kevin Bacon with 6 degrees of separation:**
``` cypher
MATCH (:Person {name:"Kevin Bacon"})-[*1..6]-(n)
RETURN DISTINCT n
```
**Shortest path between two nodes of Kevin Bacon and Meg Ryan:**
``` cypher
...
```
#### Recommendations
**Find actors who he has yet to work with, but who has worked with his previous co-actors to find someone who can introduce Tom Hanks to his potential new co-actor. Co-actors are the first degree of separation. Coco-actors are the second degree of separation. On the top of the list there will be an actor with the most connections with the second degree of separation people. Check who (he already worked with) can introduce Tom Hanks to the most people:**
``` cypher
...
```
**If we want a specific person from the second degree of separation people we input a specific name into cocoActors place. Check who can introduce Tom Hanks to Keanu Reeves:**
``` cypher
